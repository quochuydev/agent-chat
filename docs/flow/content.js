...
async function wakePromptBox() {
  ...
  if (arrow) {
    ...
    for (const [x, y] of points) {
      ...
      ed = findPromptInput();
      if (ed) {
        console.log("[cappuai_flow] ✓ Đánh thức được ô prompt bằng cú click.");
        return ed;
      }
    }
  }
...
async function submitPromptReliably(input, disabledBefore) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (inputText(input).trim().length < 3) {
      console.log("[cappuai_flow] ✓ Gửi bằng ENTER (lần", i + 1, ")");
      return true;
    }
    ...
    if (arrow) {
      ...
      await sleep(700);
      if (inputText(input).trim().length < 3) {
        console.log("[cappuai_flow] ✓ Gửi bằng CLICK nút (lần", i + 1, ")");
        return true;
      }
    }
  }
...
async function waitForNewImage(baselineSet) {
  ...
  console.log("[cappuai_flow] Đang chờ ảnh mới... (số ảnh nền ban đầu:", baselineSet.size, ")");
  let lastLog = 0;
  while (Date.now() - start < CONFIG.maxWaitMs) {
    ...
    if (fresh.length) {
      ...
      console.log("[cappuai_flow] ✓✓ Thấy ảnh mới! Đang tải:", srcKey(newest).slice(0, 70));
      return { img: newest, src: srcKey(newest) };
    }
    const elapsed = Math.round((Date.now() - start) / 1000);
    if (elapsed - lastLog >= 10) {
      lastLog = elapsed;
      console.log("[cappuai_flow] ...vẫn đang chờ ảnh —", elapsed, "giây trôi qua. Số ảnh ≥256px hiện có:", all.length);
    }
...
async function runOne(prompt) {
  ...
  if (!input) {
    console.warn(
      "[cappuai_flow] ❌ KHÔNG tìm thấy ô prompt. Đếm phần tử trong DOM →",
  ...
  console.log("[cappuai_flow] ✓ Ô prompt:", input.tagName, input.getAttribute("role"));
  ...
  console.log("[cappuai_flow] Sau khi gõ, nội dung ô =", JSON.stringify(got.slice(0, 50)), "(", got.length, "ký tự )");
  if (got.length < 3) {
    console.warn("[cappuai_flow] ❌ Gõ chữ KHÔNG vào được ô (Slate không nhận).");
  ...
  if (!sent) {
    console.warn("[cappuai_flow] ❌ Gõ được nhưng KHÔNG gửi được.");
    return {
      ok: false,
...
if (!window.__CAPPUAI_FLOW_LISTENER__) {
  window.__CAPPUAI_FLOW_LISTENER__ = true;
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === "GET_BOX") {
      ...
      (async () => {
        try {
          ...
          window.__cappuai_flow_baseline = new Set(getCompletedImages().map(srcKey));
          sendResponse({
            ok: true,
    ...
    if (msg.type === "WAIT_IMAGE") {
      const baseline = window.__cappuai_flow_baseline || new Set();
  ...
  console.log("[cappuai_flow] content script đã sẵn sàng trên Google Flow.");
}
