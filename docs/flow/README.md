# cappuai_flow — Bulk Image cho Google Flow
...
> 2. Khi chạy, Chrome hiện thanh vàng **"cappuai_flow đang gỡ lỗi trình duyệt này"** — **để yên, đừng bấm Hủy**.
>
...
4. Bấm **Tải tiện ích đã giải nén** → chọn thư mục `cappuai_flow`.
...
4. Dán prompt vào ô — **mỗi dòng một prompt** (hoặc bấm _Tải file .txt_).
...
- **Cách canh "xong":** Tool coi là xong khi có _ảnh mới xuất hiện_. Nếu bạn để sẵn nhiều ảnh cũ giống hệt, hoặc Flow load chậm, có thể nhầm. Tăng `settleMs`/`maxWaitMs` nếu cần.
...
- Đổi tiêu đề "cappuai_flow" trong `sidepanel.html` và màu trong `sidepanel.css` (sửa biến `--accent`).
